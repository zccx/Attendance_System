...
var sqlMap = {
  ...
  student:{
    ...
    select_record_1:'SELECT * from record where sno=? order by date DESC,time DESC',
    select_record_2:'SELECT * from record where sno=? and cno=? and date=?' ,
    select_course_1:'SELECT * from course where cno=?',
    select_student:'SELECT * from student where sno=?',
  ...
  teacher:{
    select_qj:'SELECT * from qingjia where cno=? and qjday=?',
    select_record:'SELECT * from record where cno=? and date=?' ,
    select_rel_tno:'SELECT * from fabu where tno=? order by date DESC,stime1 DESC',
    select_rel_date:'SELECT * from fabu where date=?',
    add_release:'insert into fabu(id,cno,course,tno,stime1,stime2,etime1,etime2,date) values(0,?,?,?,?,?,?,?,?)',
