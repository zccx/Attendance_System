<template>
  <div>
    <mt-header fixed title="请假" style="height: 50px;font-size: 18px;align-items: center">
    ...
    <div class="flex" style="margin-top: 50px">
      ...
      <div>
        <span>请假原因</span>
        <mt-field style="margin-top:20px;box-sizing: border-box;border-radius: 2px;border:1px solid #dedede;" type="textarea" rows="2" v-model="reason"></mt-field>
      </div>
    ...
    <mt-button style="width: 150px;margin-top: 30px" type="primary" @click="qingjia()">提交申请</mt-button>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods:{
      ...
      qingjia(){
        ...
        {
          if(this.items[i].name==this.course){
            cno=this.items[i].cno;
            break;
          }
        }
        var qjday=this.calendar3.value[0]+'/'+this.calendar3.value[1]+'/'+this.calendar3.value[2]
        let data1={
      ...
      openByDrop(e){
        ...
        this.calendar3.top=e.target.offsetTop-220;
